...
import { Settings, Building2, Bell, Shield, Cpu, RefreshCw, Save, Users, BarChart3, Lock, Edit, X } from 'lucide-react';
...
export default function SettingsPage() {
    ...
    const handleCancelEditAI = () => {
        setIsEditingAI(false);
        // Reset inputs back to what was fetched
        fetchAIParams();
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-1 w-fit">
                <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-md text-sm font-semibold transition-all ${activeTab === 'general' ? 'bg-white text-[#4318FF] shadow-sm' : 'text-slate-500 hover:text-slate-700'
                        }`}
                >
                ...
                <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-2 rounded-md text-sm font-semibold transition-all ${activeTab === 'ai' ? 'bg-white text-[#4318FF] shadow-sm' : 'text-slate-500 hover:text-slate-700'
                        }`}
                >
                ...
                {isAdmin && (
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-4 py-2 rounded-md text-sm font-semibold transition-all ${activeTab === 'staff' ? 'bg-white text-[#4318FF] shadow-sm' : 'text-slate-500 hover:text-slate-700'
                            }`}
                    >
                            ...
                            <div className="flex items-center gap-2">
                                {isEditingAI ? (
                                    ...
                                    <button
                                        ...
                                        disabled={aiLoading}
                                        className="flex items-center gap-1.5 text-xs font-semibold text-[#4318FF] hover:text-white bg-indigo-50 hover:bg-[#4318FF] border border-transparent px-3 py-1.5 rounded transition-all disabled:opacity-50"
                                    >
                                ...
                                <form onSubmit={handleUpdateAI} className="space-y-4">
                                    <div className="space-y-4">
                                        <div className="space-y-2">
                                            ...
                                            <input
                                                ...
                                                className="w-full px-3 py-2 border border-gray-200 rounded text-sm font-medium text-gray-900 bg-white focus:outline-none focus:ring-1 focus:ring-[#4318FF] focus:border-[#4318FF] disabled:bg-gray-50 disabled:text-gray-500"
                                    ...
                                    {isEditingAI && (
                                        <div className="flex justify-end gap-2 pt-2">
                                            <button type="button" onClick={handleCancelEditAI} disabled={updating}
                                                className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded transition-all disabled:opacity-50">
                                                <X className="w-4 h-4" />
                                                Hủy
                                            </button>
