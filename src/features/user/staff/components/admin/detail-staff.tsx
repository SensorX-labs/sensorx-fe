...
import { Mail, Phone, Briefcase, ArrowLeft, Loader2, CreditCard, Calendar, FileText, Brain } from 'lucide-react';
...
export function DetailStaff({ id }: DetailStaffProps) {
  ...
  const isSalesStaff = staff.department?.toLowerCase() === 'sale';
  ...
  return (
    <div className="space-y-6">
      ...
      {isSalesStaff ? (
        <Tabs defaultValue="info" className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start w-full">
          {/* Left Column: Profile */}
          <div className="md:col-span-1 bg-white p-6 rounded-xl shadow-sm border border-slate-100 space-y-6 h-fit shrink-0">
            ...
            <div className="pt-5 border-t border-slate-100">
              <TabsList className="flex flex-col w-full h-auto bg-transparent p-0 gap-1.5">
                ...
                >
                  <Briefcase size={15} className="mr-2.5" /> Thông tin & Công việc
                </TabsTrigger>
                <TabsTrigger
                  value="ai-metrics"
                  className="w-full justify-start px-4 py-2.5 rounded-lg data-[state=active]:bg-indigo-50/60 data-[state=active]:text-indigo-600 text-slate-600 hover:bg-slate-50 transition-all font-bold text-sm border border-transparent data-[state=active]:border-indigo-100/50"
                >
                  <Brain size={15} className="mr-2.5" /> Chỉ số năng lực AI
                </TabsTrigger>
              </TabsList>
            </div>
          </div>

          {/* Right Column: Information Data inside Tabs */}
          <div className="md:col-span-2 w-full">
            ...
            <TabsContent value="ai-metrics" className="mt-0 outline-none">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start w-full">
          ...
          {/* Right Column: Information Data directly */}
