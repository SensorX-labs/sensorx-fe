...
import { Package, Tag, AlertTriangle, TrendingUp, Eye, Edit, Trash2 } from 'lucide-react';
              ...
              {mockProducts.map((p) => (
                ...
                >
                  ...
                  <td className="px-6 py-3 font-semibold ">
                    {p.priceList?.tiers && p.priceList.tiers.length > 0
                      ? new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(p.priceList.tiers[0].defaultPrice)
                      : '--'}
                  </td>
                  ...
                  <td className="px-6 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-500 hover:text-blue-700 hover:bg-blue-50" onClick={() => router.push(`/catalog/products/${p.id}`)}>
