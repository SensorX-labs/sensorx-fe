    ...
    return (
        ...
        >
            ...
            >
                ...
                {/* Quick Add Button */}
                <div className={`absolute inset-x-4 bottom-4 z-20 transition-all duration-500 transform ${
                    isHovered ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0 pointer-events-none'
                }`}>
                    <button
                        onClick={handleAddToCart}
                        className="w-full py-3.5 bg-gray-900 text-white text-[10px] font-bold uppercase tracking-[0.3em] rounded-sm flex items-center justify-center gap-2 hover:bg-brand-green transition-all duration-300 shadow-xl"
                    >
                        <ShoppingBag size={14} />
                        Thêm nhanh
                    </button>
